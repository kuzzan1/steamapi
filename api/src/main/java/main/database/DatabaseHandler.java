        ...
        new Thread(() -> {
            ...
            try {
                ...
                if (playerId.hasNext()) {
                    ...
                    System.out.println("Updated Sport: " + discipline.getName());
                } else {
                    table.insert(discipline).optArg("conflict", "replace").run(conn);
                    System.out.println("Inserted Sport: " + discipline.getName());
                }

            } catch (Exception e) {
                System.err.println("Error: Sport failed");
            } finally {
                conn.close();
            }
        }).start();
